...
type SocioPaymentFormProps = {
  total: number;
  orderId: string; // 👈 aquí ya recibes order_code
  open: boolean;
  onClose: () => void;
};
...
const SocioPaymentForm: React.FC<SocioPaymentFormProps> = ({
  ...
  async function onPay() {
    ...
    const message = `💳 <b>PAGO SOCIO LENSHUB</b>
...
`;
    ...
    try {
      ...
      if (!r2.ok) {
        throw new Error(
          d2?.error || d2?.message || "No se pudo actualizar la orden"
        );
      }
            ...
            <>
              ...
              <label style={styles.rememberRow}>
                <input
                  ...
                  onChange={(e) =>
                    setForm((s) => ({ ...s, recordar: e.target.checked }))
                  }
                />
